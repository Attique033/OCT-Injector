package com.me.wangyuwei.particleview;
...
public class ParticleView extends View {
    ...
    private me.wangyuwei.particleview.Particle[][] mParticles = new me.wangyuwei.particleview.Particle[ROW_NUM][COLUMN_NUM];
    private me.wangyuwei.particleview.Particle[][] mMinParticles = new me.wangyuwei.particleview.Particle[ROW_NUM][COLUMN_NUM];
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < ROW_NUM; i++) {
            for (int j = 0; j < COLUMN_NUM; j++) {
                mMinParticles[i][j] = new me.wangyuwei.particleview.Particle(mStartMinP.x + (mEndMinP.x - mStartMinP.x) / COLUMN_NUM * j, mStartMinP.y + (mEndMinP.y - mStartMinP.y) / ROW_NUM * i, dip2px(0.8f));
            }
        }
        ...
        for (int i = 0; i < ROW_NUM; i++) {
            for (int j = 0; j < COLUMN_NUM; j++) {
                mParticles[i][j] = new me.wangyuwei.particleview.Particle(mStartMaxP.x + (mEndMaxP.x - mStartMaxP.x) / COLUMN_NUM * j, mStartMaxP.y + (mEndMaxP.y - mStartMaxP.y) / ROW_NUM * i, getTextWidth(mHostText + mParticleText, mParticleTextPaint) / (COLUMN_NUM * 1.8f));
            }
        }
    ...
    private void startParticleAnim() {
        ...
        for (int i = 0; i < ROW_NUM; i++) {
            for (int j = 0; j < COLUMN_NUM; j++) {
                final int tempI = i;
                final int tempJ = j;
                ValueAnimator animator = ValueAnimator.ofObject(new me.wangyuwei.particleview.LineEvaluator(), mParticles[i][j], mMinParticles[i][j]);
                animator.setDuration(mTextAnimTime + ((int) (mTextAnimTime * 0.02f)) * i + ((int) (mTextAnimTime * 0.03f)) * j);
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    @Override
                    public void onAnimationUpdate(ValueAnimator animation) {
                        mParticles[tempI][tempJ] = (me.wangyuwei.particleview.Particle) animation.getAnimatedValue();
